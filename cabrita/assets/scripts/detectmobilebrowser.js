...
const widthPercent = 1.7765;
const heightPercent = 0.5628;

function resizeCast() {
  const mainContainer = $('.main-container');
  const castContainer = $('.video-primary-iframe');
  const castContainerCabra = $('.video-secondary-iframe');
  const castContainerChat = $('.chat-iframe');
  const availableHeight = $(window).height() - 46;
  const availableWidth = mainContainer.width() * 0.8;

  const newHeight = availableWidth * heightPercent;
...
$(document).ready(() => {
  const check = window.mobileAndTabletCheck();
  if (check) {
